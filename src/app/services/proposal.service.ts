...
export class ProposalService implements OnDestroy {
  ...
  private proposalsUnsub?: () => void;
  private votesUnsub?: () => void;
  ...
  private subscribeToRealtimeUpdates(): void {
    this.pocketbaseService.client
      .collection('proposals')
      .subscribe('*', () => this.loadProposals())
      .then(unsub => this.proposalsUnsub = unsub);

    this.pocketbaseService.client
      .collection('votes')
      .subscribe('*', () => this.loadProposals())
      .then(unsub => this.votesUnsub = unsub);
  }
  ...
  private async loadProposals(): Promise<void> {
    try {
      const proposals = await this.pocketbaseService.client
        .collection('proposals')
        .getFullList<Proposal>({ sort: '-created_at' });

      const updatedProposals = proposals.map(p => this.updateProposalStatus(p as Proposal));
  ...
  async hasUserVoted(proposalId: string, voterId: string): Promise<boolean> {
    ...
    try {
      const votes = await this.pocketbaseService.client.collection('votes').getFullList({
        filter: `proposal_id="${proposalId}" && voter_id="${voterId}"`
      });

      const hasVotedInDb = votes.length > 0;
  ...
  async getUserVote(proposalId: string, voterId: string): Promise<'yes' | 'no' | null> {
    ...
    try {
      const votes = await this.pocketbaseService.client.collection('votes').getFullList<{vote: 'yes' | 'no'}>({
        filter: `proposal_id="${proposalId}" && voter_id="${voterId}"`,
        fields: 'vote'
      });

      const dbVoteChoice = votes.length > 0 ? votes[0].vote : null;
